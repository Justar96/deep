...
import { createTestConfig, createTestEvent, createTestConversationState } from '../../../../test-utils/test-config'

// Create fresh mock configuration for each test to ensure isolation
const createMockConfig = (): DeepConfig => createTestConfig({
  apiKey: 'test-api-key',
  model: 'gpt-5'
})
...
describe('DeepEngine', () => {
  ...
  describe('processMessage', () => {
    beforeEach(() => {
      ...
      const mockEvents = [
        createTestEvent('turn_start', { conversationId: 'test-uuid-1234', input: 'test input' }),
        createTestEvent('response_start', { responseId: 'resp-123' }),
        createTestEvent('content_delta', { text: 'Hello' }),
        createTestEvent('turn_complete', { usage: {}, responseId: 'resp-123' })
      ]
  ...
  describe('conversation management', () => {
    ...
    const createMockConversation = (config: DeepConfig): ConversationState => createTestConversationState({
      id: 'conv-123',
      lastResponseId: null,
      metrics: {
  ...
  describe('processMessageStream', () => {
    it('should delegate to processMessage', async () => {
      const mockEvents = [
        createTestEvent('turn_start', { conversationId: 'test', input: 'test input' }),
        createTestEvent('turn_complete', { usage: {}, responseId: 'test-response' })
      ]
