// Test suite for Tool Registry - Sprint 2 Enhanced Tool System Integration
import { describe, it, expect, beforeEach, vi, afterEach } from 'vitest'
import { ToolRegistry } from '@deep-agent/core'
...
describe('ToolRegistry', () => {
  ...
  describe('Tool Execution with Confirmation', () => {
    ...
    it('should request confirmation for high-risk tool', async () => {
      ...
      const testRegistry = new ToolRegistry(config)
      const events: DeepEvent[] = []
  ...
  describe('Impact Analysis', () => {
    it('should analyze tool impact', async () => {
      ...
      expect(impact.filesAffected.some(path => path.includes('test.txt'))).toBe(true)
  ...
  describe('Emergency Stop', () => {
    ...
    it('should reset emergency stop', async () => {
      // Create a test config with confirmations disabled to prevent hanging
      const testConfig = { ...mockConfig, tools: { ...mockConfig.tools, confirmationEnabled: false } }
      const testRegistry = new ToolRegistry(testConfig)

      await testRegistry.emergencyStop()
      ...
      testRegistry.resetEmergencyStop()
  ...
  describe('Tool Management Utilities', () => {
    ...
    it('should track active executions', async () => {
      ...
      testRegistry.registerTool(slowTool, executor, true)

      const executionPromise = testRegistry.executeToolCall(
        'slow_execution',
        '{}',
        'slow-call'
      )
      ...
      const activeExecutions = testRegistry.getActiveExecutions()
      ...
      const completedExecutions = testRegistry.getActiveExecutions()
  ...
  describe('Security Report', () => {
    it('should generate security report', async () => {
      ...
      await testRegistry.executeToolCall('get_current_time', '{"timezone": "UTC"}', 'safe-call')

      const report = testRegistry.getSecurityReport()
